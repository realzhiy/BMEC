...
cc.cardiac <- readRDS("cellchat_cardiac.rds")
cc.t2d <- readRDS("cellchat_t2d.rds")

data.dir <- './hs_c_comparison'
dir.create(data.dir)
setwd(data.dir)

# Merge
object.list <- list(cardiac = cc.cardiac, t2d = cc.t2d)
...
pdf("PECAM1_t2d.pdf")
par(mfrow=c(1,1))
netAnalysis_signalingRole_network(cc.t2d, signaling = pathways.show, width = 8, height = 2.5, font.size = 10)
dev.off()

library(NMF)
library(ggalluvial)
nPatterns = 3
cellchat <- identifyCommunicationPatterns(cc.t2d, pattern = "incoming", k = nPatterns)
pdf("river_incoming_t2d.pdf")
