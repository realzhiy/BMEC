...
cc.noncardiac <- readRDS("cellchat_noncardiac.rds")
cc.t2d <- readRDS("cellchat_t2d.rds")

data.dir <- './hs_nc_comparison'
dir.create(data.dir)
setwd(data.dir)


# Merge
object.list <- list(noncardiac = cc.noncardiac, t2d = cc.t2d)
...
for (i in 1:length(object.list)) {
  netVisual_circle(object.list[[i]]@net$weight, weight.scale = T, label.edge= F, edge.weight.max = weight.max[2], edge.width.max = 12, title.name = paste0("Interaction strength - ", names(object.list)[i]))
}
dev.off()
